...
weather <- all |> 
  ...
  dplyr::mutate(var = ifelse(var == "panom", prcp_season, tmp_season))

name_key <- tibble::tribble(
  ~var, ~var_lab,
  "ed_250", "Edge (250 m)",
  "ed_3000", "Edge (3000 m)", 
  "panom_br", "Precip (breeding)",
  "panom_nb", "Precip (nonbreeding)",
  "pgrass_250", "Grassland (250 m)",
  "pgrass_3000", "Grassland (3000 m)",
  "tanom_br", "Temp (breeding)", 
  "tanom_nb", "Temp (nonbreeding)")
...
trnd <- all |> 
  ...
  dplyr::mutate(var = ifelse(var == "panom", prcp_season,
                             ifelse(var == "tanom", tmp_season, var))) |> 
