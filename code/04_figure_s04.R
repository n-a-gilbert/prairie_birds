...
fit_sp <- dplyr::bind_rows(fit_sp_list) |> 
  dplyr::filter(tmp_season == "tanom_nb") |> 
...
stars <- dplyr::bind_rows(star_list) |> 
  ...
  dplyr::filter(!grepl("3000", var)) |> 
  dplyr::filter(tmp_season == "tanom_nb") |> 
  dplyr::filter(! (var %in% c("panom", "tanom") & hab_name != "pgrass_250" )) |> 
  mutate(lab = ifelse( sig95 == 1, "**", 
                       ifelse(sig95 == 0 & sig68 == 1, "*", ""))) |> 
  dplyr::ungroup() |> 
  dplyr::select(hab_name = var, lab) |> 
  dplyr::left_join(name_key) |> 
...
( panel_a <- ggplot() +
    geom_line( data = fit_sp, 
               aes(x = habitat, y = mean, group = sp),
               alpha = 0.2,
               linewidth = 0.4,
               color = MetBrewer::MetPalettes$Hiroshige[[1]][10]) +
    geom_ribbon( data = fit_com,
                 aes(x = habitat, ymin = l95, ymax = u95), 
                 color = NA,
                 fill = "#a62517",
                 alpha = 0.4 ) +
    geom_line( data = fit_com,
               aes(x = habitat, y = mean), 
               color = "#a62517",
               linewidth = 1) +
    geom_text( data = stars, aes(x = habitat, y = mean, label = lab)) +
    facet_wrap(~var_lab, scales = "free_x", nrow = 1) +
...
fit_trnd_com <- dplyr::bind_rows(fit_trnd_com_list) |> 
  dplyr::filter(tmp_season == "tanom_nb") |> 
  dplyr::filter(!grepl("3000", hab_name)) |>
  dplyr::left_join(name_key) |> 
  dplyr::mutate(var_lab = factor(var_lab, 
...
fit_trnd_sp <- dplyr::bind_rows(fit_trnd_sp_list) |> 
  dplyr::filter(tmp_season == "tanom_nb") |> 
  dplyr::filter(!grepl("3000", hab_name)) |>
  dplyr::left_join(name_key) |> 
  dplyr::mutate(var_lab = factor(var_lab, 
...
stars_trnd <- dplyr::bind_rows(star_list) |> 
  dplyr::filter(grepl("yr", var)) |>
  dplyr::filter(! (var %in% c("panom.yr", "tanom.yr") & hab_name != "pgrass_250" )) |> 
  dplyr::filter(!grepl("3000", var)) |> 
  dplyr::filter(tmp_season == "tanom_nb") |> 
  dplyr::filter(!var == "yr") |> 
  mutate(lab = ifelse( sig95 == 1, "**", 
                       ifelse(sig95 == 0 & sig68 == 1, "*", ""))) |> 
  dplyr::ungroup() |> 
  dplyr::mutate(var = stringr::str_remove_all(var, ".yr")) |> 
  dplyr::select(hab_name = var, lab) |> 
  dplyr::left_join(name_key) |> 
  dplyr::left_join(xpos) |> 
  dplyr::mutate(var_lab = factor(var_lab, 
...
( panel_b <- ggplot() +
    geom_hline(yintercept = 0.5, color = "black", linetype = "dashed",
               linewidth = 0.2) +
    geom_line( data = fit_trnd_sp, 
               aes(x = habitat, y = test, group = sp),
               alpha = 0.2,
               linewidth = 0.4,
               color = MetBrewer::MetPalettes$Hiroshige[[1]][10]) +
    geom_line( data = fit_trnd_com,
               aes(x = habitat, y = test), 
               color = "#a62517",
               linewidth = 1) +
    geom_text( data = stars_trnd, aes(x = habitat, y = mean, label = lab)) +
    facet_wrap(~var_lab, scales = "free_x", nrow = 1
    ) +
    ...
    labs(x = "Covariate value", 
         y = "Probability of decline in\n occurrence from 2008 to 2023") +
    ggplot2::theme(axis.line = element_line( linewidth = 0.15, color = "black"),
                   panel.grid = element_line(linewidth = 0.1, color = "gray80"),
